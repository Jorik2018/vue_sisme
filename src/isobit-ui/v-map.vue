...
export default {
    ...
    methods: {
        ...
        addFeature(f,cfg) {
            ...
            var coordinate = c[0] < -10000 ? ol.proj.toLonLat(c) : c;
            me.$emit('translateend', {lon: coordinate[0], lat: coordinate[1]});
			f.geometry=point;
        ...
        build(){
            ...
            let pageLocation = m.urlParam('location');
            var zoom=-1;
            if (pageLocation) {
            ...
            vp.addEventListener("parentResize",()=>{
                setTimeout(()=>{
                    map.updateSize();
