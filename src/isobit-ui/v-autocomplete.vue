<template>
	<div class="v-autocomplete" v-on:focusout2="focusout" :value="value" v-bind:disabled="disabled" >
        ...
        <input v-if="!selected" v-on:keyup="keyup" v-on:keyup.enter.stop.prevent="search"
            ...
            v-bind:disabled="disabled" v-bind:placeholder="placeholder" v-model="query">
            <button class="hide" onclick="return false"></button>
		<transition name="fade">
			<div v-show="show&&!selected" class="v-resultpane" v-bind:style="{zIndex:2,width: 'calc(100% - 2px)',position: (floating?'absolute':'relative')}" 
				...
				<div class="v-list">
					<div v-for="(d,i) in data2" :key="i" v-bind:class="{'v-selected':selected==d,'v-focused':focused==i}"  v-on:click="setSelected(d)">
...
export default {
    ...
    data() {
        return {
            page: 0, selected: null, focused: -1, query: null,
            data0: null,
            setData: {},
            show: false, inputClass: ''
        }
    },
    ...
    updated() {
        ...
        if (me.data) {
            this.setData[this.keySet] = me.data;
            //me.data0 = me.data;
        }
    },
    watch: {
        keySet(v){
            let d=this.setData[v];
            if(!d)d={};
            this.data0=d.data;
        ...
        search(e) {
            ...
            axios.get((me.src ? me.src : me.source) + (pagination > 0 ? '/' + me.page + '/' + pagination : ''), { params: params })
                ...
                if(!d.error){
                    ...
                    me.total = d.size;
                    me.setData[me.keySet] = {data:me.data0,total:me.total };
