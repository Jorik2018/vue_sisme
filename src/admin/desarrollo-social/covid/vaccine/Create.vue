	...
	<v-form header="VACUNAS COVID" action="/admin/desarrollo-social/covid"> 
		<div> 
			<v-fieldset legend="BUSQUEDA POR DOCUMENTO IDENTIDAD" class="v-form"> 
				...
				minQueryLength="5" required="required" v-model="o" v-on:complete="peopleComplete" src="/admin/desarrollo-social/api/covid/vaccine-covid"> 
				...
				<template v-slot="{row}">
				{{row.numDoc}}: {{row.paciente}} - {{row.dosisAplicada}} {{row.fabricanteAbrev}} en {{row.fechaVacunacion}}
				</template> 
				</v-autocomplete> 
			...
			<v-fieldset v-if="o" legend="Resultados"> 
				<div v-for="(o,j) in l" v-bind:key="j" class="v-item" style="padding: 5px;background-color: black;color:white;line-break: anywhere;">
					...
					<div>
					{{o.dosisAplicada}} {{row.fabricanteAbrev}}
					</div>
