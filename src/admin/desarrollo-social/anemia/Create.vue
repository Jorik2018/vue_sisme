<template>
	<v-form v-bind:header="(o.id?'Editar ':'Crear ')+' Control Anemia'" action="/admin/desarrollo-social/api/hemoglobin"> 
 <div class="v-form"> <label for="dni">DNI / CUI:</label> 
  <input type="tel" pattern="[0-9]{8}" class="center" maxlength="9" id="dni" v-model="o.ext.peopleId" required="required" /> 
  <div id="msg"></div> 
  <div class="right" style="margin-top:10px"> <v-button icon="fa-sync" v-on:click="recoverData" id="recover" value="Recuperar Datos"></v-button> 
  </div> <label>Sexo:</label> <v-radio-group required="true" v-model="o.sex"> 
   <v-radio label="Masculino" value="M"></v-radio> 
   <v-radio label="Femenino" value="F"></v-radio> 
  </v-radio-group> <label for="birthDate">Fecha de Nacimiento:</label> <v-calendar v-model="o.birthDate" name="birthDate" required="required"></v-calendar> 
  <div id="dateDiff" v-if="dateDiff" style="width:unset">
   {{dateDiff}}
  </div> <v-fieldset> 
   <small class="form-text text-muted block">Registrar datos del carnet del ni&ntilde;o.</small> 
   <label>Peso (Kg.):</label> 
   <input type="number" v-model="o.weight" required="true" min="0" /> 
   <label>Altura (Cm):</label> 
   <input type="number" v-model="o.height" min="0" required="true" step="1" /> 
  </v-fieldset> <v-fieldset legend="LABORATORIO (Hb OBSERVADA) *"> 
   <small class="form-text text-muted block">Registrar la HEMOGLOBINA OBSERVADA, ya que el sistema realiza el ajuste autom&aacute;tico. Si se registra la Hemoglobina ajustada ser&aacute; otra vez ajustada.</small> 
  ...
  <input type="number" v-model="o.controls" min="0" /> <v-fieldset legend="DESDE LA CONSULTA ANTERIOR"> 
   ...
   <v-select v-model="o.province" name="sidProvincia" required="required" v-on:input="$refs.districtSelect.load({provinceId:parseInt(o.province)});"> 
    <option value="">Select One...</option> 
    <v-options url="/admin/directory/api/province/0/0?regionId=2" value-field="code" display-field="name"></v-options> 
   </v-select> 
   <label>Distrito:</label> 
   <v-select v-bind:disabled="!o.province" name="sidDistrito" v-model="o.district" required="required" v-on:input="$refs.establishmentSelect.load({location:o.sidDistrito+'%'})"> 
    <option value="">Select One...</option> 
    <v-options ref="districtSelect" url="/admin/directory/api/district/0/0" value-field="code" display-field="name"></v-options> 
   </v-select> 
   ...
   <div id="gallery"> 
    <img v-for="(file,i) in o.ext.files" v-bind:key="i" v-bind:id="file.id" v-bind:src="file.src" v-bind:class="{synced:file.synced}" v-on:click="sync(file)" /> 
   </div> 
   <div class="right"> 
