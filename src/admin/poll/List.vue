<template>
    <v-form
      header="Cuestionario de Condiciones de Vida"
      ...
      >
        ...
        <template v-slot:default="{ row }">
          ...
          <td header="IDENTIFICACION MUESTRAL" width="300">
            <a v-bind:href="'/admin/poll/' + (row.tmpId ? -row.tmpId : row.id)"
              >URB_RUR={{ row.urb_rur }}
              <template v-if="row.distrito"
                >/ DISTRITO={{ row.distrito }}</template
              >
              <template v-if="row.conglomerado"
                >/ CONGLOMERADO={{ row.conglomerado }}</template
              >
              <template v-if="row.zona">/ ZONA={{ row.zona }}</template>
              <template v-if="row.manzana">/ MANZANA={{ row.manzana }}</template>
              <template v-if="row.vivienda"
                >/ VIVIENDA={{ row.vivienda }}</template
              >
              <template v-if="row.hogar">/ HOGAR={{ row.hogar }}</template>
            </a>
          </td>
          <td header="DIRECCION" width="200">
            <template v-if="row.nombre_calle"
              >/ CALLE O AVENIDA={{ row.nombre_calle }}</template
            >
